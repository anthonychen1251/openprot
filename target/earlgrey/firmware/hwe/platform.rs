...
fn setup_spi_mux_for_flash0() -> Result<(), ErrorCode> {
    ...
    GpioPort::configure(&mut gpio_port, pin8_mask, config8)
        .map_err(|_| ErrorCode::new(0x464c0001))?;
    ...
    GpioPort::configure(&mut gpio_port, pin9_mask, config9)
        .map_err(|_| ErrorCode::new(0x464c0002))?;

    // Set both GPIO 8 and GPIO 9 to Low (0) to select Flash 0 and enable the Mux
    let pins_mask = earlgrey_gpio::GpioMask((1 << 8) | (1 << 9));
    GpioPort::set_reset(
        &mut gpio_port,
        earlgrey_gpio::GpioMask(0), // set_mask (High)
        pins_mask,                  // reset_mask (Low)
    )
    .map_err(|_| ErrorCode::new(0x464c0003))?;
